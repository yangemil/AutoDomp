...
import { TestCase, TestExecution, TestStepResult, ProjectSettings, ScenarioExecution, PerformanceMetrics, EnvironmentInfo } from '../../common/interfaces';
...
export class TestEngineService {
  private readonly logger = new Logger(TestEngineService.name);
  private browser: Browser | null = null;
  private activeExecutions: Map<string, TestExecution> = new Map();
  private scenarioContexts: Map<string, { browser: Browser; context: BrowserContext; page: Page }> = new Map();
  ...
  private async readCsvData(csvPath: string, dataRange?: string): Promise<Record<string, any>[]> {
  ...
  private replaceVariables(value: string, data: Record<string, any>): string {
  ...
  private async executeSingleTest(
    testCase: TestCase,
    config: ProjectSettings & { baseUrl: string },
    ...
    if (Object.keys(mergedTestData).length > 0) {
      ...
      // 新增：使用结构化日志
      const structuredLog = this.loggingService.logTestExecution(
        execution.id,
        LogLevel.INFO,
        logMessage
      );
    ...
    try {
      ...
      if (testCase.url) {
        ...
        try {
          ...
          // 新增：记录页面加载时间和性能
          execution.structuredLogs?.push(
            this.loggingService.logTestExecution(
              execution.id,
              LogLevel.INFO,
              '页面加载成功',
              {
          ...
          execution.structuredLogs?.push(
            this.loggingService.logError(
              ...
              navError as Error,
              { projectId: testCase.projectId },
              { url: initialUrl, action: 'navigate' }
            )
          );
        }
      }
      ...
      for (let i = 0; i < testCase.steps.length; i++) {
        ...
        // 新增：使用结构化日志
        const structuredLog = this.loggingService.logStepExecution(
          execution.id,
          step.order,
          LogLevel.DEBUG,
          logMessage,
          { action: actionText, selector: step.selector, value: step.value }
        );
        execution.structuredLogs?.push(structuredLog);
            ...
            try {
              ...
              execution.structuredLogs?.push(
                this.loggingService.logTestExecution(
                  ...
                  LogLevel.ERROR,
                  logMessage,
                  { error: { message: screenshotError.message, type: screenshotError.constructor.name } }
                )
              );
            }
          }
      ...
      if (execution.performanceMetrics) {
        ...
        // 找出最慢步骤
        const slowestStep = execution.steps.reduce((max, s) =>
  ...
  async executeTestCase(testCaseId: string, options?: any, scenarioContext?: Record<string, any>, sharedPage?: Page): Promise<TestExecution | TestExecution[]> {
    ...
    const mergedConfig: ProjectSettings & { baseUrl: string } = {
      ...project.settings,
      baseUrl: project.baseUrl,
      ...options
    };

    let testDataRows: Record<string, any>[] = [];
  ...
  async executeScenario(scenarioId: string, options?: any, projectId?: string): Promise<ScenarioExecution> {
    ...
    for (let i = 0; i < scenarioTestCases.length; i++) {
      ...
      try {
        const testExecutionResult = await this.executeTestCase(
          testCaseId,
          config,
          {},
          sharedPage
  ...
  async createScenarioSession(scenarioId: string, config: ProjectSettings & { baseUrl: string }): Promise<void> {
    if (this.scenarioContexts.has(scenarioId)) {
      return;
    }

    // 每次创建新会话都使用最新配置，关闭旧浏览器
    if (this.browser) {
      await this.browser.close();
      this.browser = null;
    }
  ...
  async closeScenarioSession(scenarioId: string): Promise<void> {
    const session = this.scenarioContexts.get(scenarioId);
    if (session) {
    ...
    try {
      ...
      if (step.selector && step.highlightOnScreenshot) {
        ...
        await page.evaluate((sel) => {
          ...
          if (el) {
            (el as HTMLElement).style.border = '5px dashed #ff0000 !important';
            (el as HTMLElement).style.boxShadow = '0 0 10px #ff0000';
  ...
  // 处理带序号的文本选择器
  private async resolveTextSelector(page: Page, selector: string): Promise<Locator> {
    // 格式: text:nth=2,请选择 或 text:第2个,请选择 或 text:倒数第2个,请选择
    let index = 0; // 0 表示第一个
  ...
  private async resolveSelectorWithAI(page: Page, selector: string): Promise<string> {
    // 如果不是 AI 选择器，使用原有的解析逻辑
  ...
  private async smartClick(page: Page, selector: string, timeout: number): Promise<void> {
    if (!selector.startsWith('smart=')) {
   ...
   * 新增：收集环境信息
   */
  private async collectEnvironmentInfo(page: Page): Promise<EnvironmentInfo> {
    const os = require('os');
